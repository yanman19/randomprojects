...
def load_data():
    url = "https://fbref.com/en/comps/19/Europa-League-Stats"
    ...
    df['Country'] = 'Europa'
    # Fix the squad name extraction to handle potential non-string types
...
# Fixed line:
teams = sorted([str(team) for team in home['Squad'].unique()])  # Convert all items to strings before sorting
...
app.layout = dbc.Container([
    html.H1("YANUS Europa", style={'textAlign': 'center', 'marginTop': 20, 'userSelect': 'none'}),
...
def refresh_data(n_clicks):
    if n_clicks:
        ...
        teams_list = sorted(home_df['Squad'].unique().tolist())  # Convert to list for proper sorting
    ...
    if tab == 'tab-match':
        ...
        return html.Div([
            ...
            html.Ul([
                ...
                html.Li("Does not account for where the teams come from since this logic deems all teams are equal since they are 'in the Europa League.'", style={'userSelect': 'none'}),
                html.Li("Assumes only performance data from this year in the Europa League.", style={'userSelect': 'none'})
            ], style={'textAlign': 'left', 'maxWidth': '800px', 'margin': '0 auto', 'userSelect': 'none'}),
            html.H3("Next Steps", style={'marginTop': '30px', 'userSelect': 'none'}),
            html.Ul([
                html.Li("Combine with big 5 league model.", style={'userSelect': 'none'}),
                html.Li("Create a weighted wxG where i can toggle how much a team should be using its Europa or league wxG. (Europa wxG will not be adjusted for any conversion rate)", style={'userSelect': 'none'}),
                html.Li("For teams who are not in top 5, assume their Europa wxG since there is no other way to get their data.", style={'userSelect': 'none'})
            ], style={'textAlign': 'left', 'maxWidth': '800px', 'margin': '0 auto', 'userSelect': 'none'}),
        ])
